...
class SensorExCard extends LitElement {
  ...
  _bandRuns(points, xOf, yOf, fallback) {
    ...
    // A colour can only change where a range begins or ends.
    const bounds = [
      ...new Set(
        rules
          .filter(Boolean)
          .flatMap((rule) => [rule.value_from, rule.value_to])
          .map(Number)
    ...
    const add = (color, point) => {
      ...
      if (!run || run.color !== color) {
        // A new run opens on the previous point, so consecutive runs share a
        // vertex and their fills tile without a seam.
    ...
    for (let i = 0; i < points.length - 1; i += 1) {
      ...
      for (let k = 0; k < stops.length - 1; k += 1) {
        const from = stops[k];
        const to = stops[k + 1];
        // Midpoint of the sub-segment decides its colour, so gaps and overlaps
        // between rules resolve exactly as they do for the readout.
  ...
  render() {
    ...
    const labelFontSize =
      Number(format.label_size) ||
      Number(format.label_font_size) ||
      Number(this.config.label_font_size) ||
      18;
    ...
    // Assembled as one run with nothing between the tspans: whitespace inside
    // an SVG <text> renders as a space, so any newline here would reappear as
    // a gap between the number and its decimals.
    const valueTspans = svg`<tspan style="font-size: ${valueFontSize}px${
      valueColor ? `; fill: ${valueColor}` : ""
    }">${valueParts.whole}</tspan>${
      valueParts.fraction
        ? svg`<tspan class="decimal" style="font-size: ${decimalFontSize}px${
            valueColor ? `; fill: ${valueColor}` : ""
      ...
      !trend
        ? svg``
        : svg`<tspan class="trend" dx="6" style="font-size: ${trendFontSize}px${
            this._trendColor(trend.direction)
              ? `; fill: ${this._trendColor(trend.direction)}`
              : ""
