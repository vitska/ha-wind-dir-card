...
class DistributionExCard extends LitElement {
  ...
  _flows() {
    ...
    const home =
      homeEntity !== null ? homeEntity : solarToHome + gridToHome + batteryToHome;
      ...
      <ha-card>
        ...
        >
          <svg viewBox="0 0 ${this._width} ${this._height}">
            ...
            ${nodes.map((node) =>
              this._renderNode(node, geo, flows.totals[node], unit)
            )}
          </svg>
          ${this.config.show_icons === false
            ...
            : nodes.map(
                (node) => html`
                  ...
                  >
                    <ha-icon
                      .icon=${this.config[`${node}_icon`] ||
                      NODE_DEFAULTS[node].icon}
                    ></ha-icon>
                  </div>
                `
              )}
